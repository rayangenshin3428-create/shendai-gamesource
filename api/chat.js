...
// Modèle Llama 3.1 70B via Groq (gratuit, très rapide, inscription par email).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!text) {
      res.status(502).json({ error: 'Réponse vide de Groq.', detail: data?.error ?? data?.message ?? null });
      return;
    }
