...
## 0. ÉTAT : DÉJÀ PRÉ-BRANCHÉ ✅ (Groq/Llama 3.1 70B via route serverless `/api/chat`)
...
- `api/chat.js` — **route serverless** (Vercel) : proxy vers l'API Groq
  (OpenAI-compatible). Lit la clé dans `process.env.GROQ_API_KEY` (jamais
...
1. obtenir une clé sur **https://console.groq.com/** (gratuit, sans CB, inscription email) ;
2. la mettre dans **Vercel → Settings → Environment Variables → `GROQ_API_KEY`** ;
3. (re)déployer.

Le modèle est dans `api/chat.js` (`MODEL`, `llama-3.1-70b-versatile`) — ajustable.
...
// model: 'llama-3.1-70b-versatile', messages: [{role:'system',...}, ...historique],
